...
const server = 'https://www.nbrb.by/api/exrates/rates/431'; //адрес сервера
const response = await fetch(server, {
    method: 'GET',
});
const responseResult = await response.json(); //перевод полученной строки в json-объект
if (response.ok) {getRate(responseResult, timerID);} //вызов функции при успехе
else {console.log(responseResult.message);} //вывод ошибки
